...
*   **Gemini**: 🧠 **多面手专家（可选）**
    *   与 Claude 同等级别的顶级 AI 专家，按需调用。可担任高阶顾问、独立审核者。
*   **Frontend**: 🎨 **前端/UI 专家**
    *   专注于界面设计、样式实现、响应式布局、动效开发，设计师型开发者视角。
*   **Chore**: 🔧 **杂务执行者**
    *   处理简单、重复、消耗 token 多的杂务任务，如批量重命名、文本替换、格式化等。
*   **Librarian**: 📚 **深度研究专家**
    *   代码搜索 + 网络研究 + 文档查询，集成 context7/websearch/github/firecrawl MCP。
*   **Looker**: 👁️ **多模态分析专家**
    *   分析 PDF、图片、图表、架构图、截图等媒体文件。
...
调用 Gemini CLI (gemini-3-pro) 进行专业的前端/UI 开发，设计师型开发者视角。
...
| `max_duration` | int | - | `1200` | 总时长硬上限（秒，20 分钟） |
| `max_retries` | int | - | `1` | 最大重试次数 |

**角色定位**：
- 🎨 **界面设计**：页面布局、组件设计
- 💄 **样式实现**：CSS、Tailwind、动效
- 📱 **响应式适配**：多设备/多分辨率
- ✨ **UI 审查**：间距、色彩、排版评审

**支持技术栈**：React/Next.js/Vue/Nuxt/Svelte/HTML+Tailwind

### `chore` - 杂务执行者

调用可配置后端（与 Coder 相同）执行简单、重复、消耗 token 多的杂务任务。
...
| `max_duration` | int | - | `600` | 总时长硬上限（秒，10 分钟） |
...
调用 Gemini CLI (gemini-3-flash) 进行代码搜索和网络研究，集成多个 MCP 工具。

| 参数 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :---: | :--- | :--- |
| `PROMPT` | string | ✅ | - | 研究任务描述 |
| `cd` | Path | ✅ | - | 工作目录 |
| `sandbox` | string | - | `read-only` | 沙箱策略（只读） |
| `timeout` | int | - | `120` | 空闲超时（秒） |
| `max_duration` | int | - | `600` | 总时长硬上限（秒） |
| `max_retries` | int | - | `1` | 最大重试次数 |
...
调用 Gemini CLI (gemini-3-flash) 进行媒体文件分析。

| 参数 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :---: | :--- | :--- |
| `file_path` | string | ✅ | - | 要分析的媒体文件路径 |
| `goal` | string | ✅ | - | 分析目标 |
| `cd` | Path | ✅ | - | 工作目录 |
| `sandbox` | string | - | `read-only` | 沙箱策略（只读） |
| `timeout` | int | - | `120` | 空闲超时（秒） |
| `max_duration` | int | - | `300` | 总时长硬上限（秒，5 分钟） |
| `max_retries` | int | - | `1` | 最大重试次数 |
