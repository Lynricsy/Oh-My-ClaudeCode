...
if ($DryRun) {
    ...
    if ($geminiInstalled) {
        try {
            ...
            Write-Host ""
            Write-Host "============================================================" -ForegroundColor White
            Write-Host "Context7 API Key (for context7 MCP) [Optional]" -ForegroundColor Yellow
            Write-Host "============================================================" -ForegroundColor White
            Write-Host ""
            Write-Host "How to get your API key:"
            Write-Host "  1. Go to: https://context7.com/"
            Write-Host "  2. Sign up or log in to your account"
            Write-Host "  3. Navigate to Dashboard -> API Keys"
            Write-Host "  4. Click 'Create API Key'"
            Write-Host "  5. Copy the generated API key (starts with 'ctx7sk-')"
            Write-Host ""
            Write-Host "  Note: Context7 works without API key but with rate limits."
            Write-Host "        API key provides higher quotas and priority access."
            Write-Host "        This is OPTIONAL - you can skip if you don't have one."
            Write-Host ""
            Write-Host "============================================================" -ForegroundColor White
            Write-Host ""
            ...
            if (Test-Path $geminiEnvPath) {
                ...
                if ($env:CONTEXT7_API_KEY) { $context7Key = $env:CONTEXT7_API_KEY }
            }
            ...
            if ($context7Key) {
                Write-Success "CONTEXT7_API_KEY already set"
                $updateContext7 = Read-Host "Update it? (y/N)"
                if ($updateContext7 -eq "y" -or $updateContext7 -eq "Y") {
                    $secureKey = Read-Host "Enter your Context7 API Key" -AsSecureString
                    $context7Key = [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($secureKey))
                    if ($context7Key) {
                        $envUpdated = $true
                    }
                }
            } else {
                $configContext7 = Read-Host "Configure CONTEXT7_API_KEY now? (optional, press Enter to skip)"
                if ($configContext7 -eq "y" -or $configContext7 -eq "Y") {
                    $secureKey = Read-Host "Enter your Context7 API Key" -AsSecureString
                    $context7Key = [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($secureKey))
                    if ($context7Key) {
                        $envUpdated = $true
                        Write-Success "Context7 API key configured"
                    } else {
                        Write-WarningMsg "Skipped Context7 API key (context7 will work with rate limits)"
                    }
                } else {
                    Write-WarningMsg "Skipped Context7 API key (context7 will work with rate limits)"
                }
            ...
            if ($githubToken -or $firecrawlKey -or $context7Key) {
                $envContent = @"
...
"@
                ...
                if ($context7Key) {
                    $envContent += "`$env:CONTEXT7_API_KEY = `"$context7Key`"`n"
                }
